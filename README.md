...
Learn about Sandpiper at sandpiperframework.org
 ...
 Sandpiper servers use an authentication mechanism around JWTs (JSON Web Tokens). The /login route accepts a POST containing username, password and plandocument and (on successful auth) return a JWT. The JWT's "payload" section enumerates the specific of the client/server relationship - including the endpoints the client may access on the server.
