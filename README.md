...
This client does Level-1 (full-file) grain interaction with the server. The local pool (and all its Content) is contained inside a local SQLite database file. The role (primary/secondary) of the client determines the direction (push/pull) that content will flow between the local and remote pools. Grains can be exported to the local filesystem.
